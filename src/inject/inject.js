chrome.extension.sendMessage({}, function(response) {
	...
	if (document.readyState === "complete") {
		...
		// below, we will separately remove the posts from the content pages and the topics fromthe main page
		// we handle this two cases differently as we can only rely on the DOM and the DOM is very different in the content page and main page
			...
			$(".username").each(function(index, element) {
				...
				if(text === name) {
					...
					if (recentTitle) {
						...
						console.log("Removed element: " + granpa);
					}
					granpa.remove();
				}
			});
